...
public:
    ...
    int ExtractMin()
    {
        ...
        while (currentPosition < queue[0] && queue[currentPosition] > queue[currentPosition * 2])
        {
    ...
    // a b-tree has 2^(n) nodes at each level.
    // the height should be floor ( log2(total nodes + 1) )
    void PrintQueue()
    {
        unsigned int height = floor(log2(queue[0]) + 1);
